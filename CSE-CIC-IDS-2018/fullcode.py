...
# Ensure CatBoost writes training metadata/logs to node-local temp when available
# Use JOBLIB_TEMP_FOLDER (set by run.slurm to SLURM_TMPDIR) or TMPDIR as fallback.
_cb_temp_root = Path(os.environ.get("JOBLIB_TEMP_FOLDER") or os.environ.get("TMPDIR") or "/tmp")
CATBOOST_TRAIN_DIR = _cb_temp_root / f"catboost_info_job{os.getpid()}"
try:
    CATBOOST_TRAIN_DIR.mkdir(parents=True, exist_ok=True)
except Exception:
    # Best-effort: if we cannot create it, fall back to /tmp (still better than NFS)
    CATBOOST_TRAIN_DIR = Path("/tmp") / f"catboost_info_job{os.getpid()}"
    CATBOOST_TRAIN_DIR.mkdir(parents=True, exist_ok=True)
# Check whether we can actually write into the chosen directory; fallback to disabling writes if not
CATBOOST_CAN_WRITE = False
try:
    if CATBOOST_TRAIN_DIR.exists() and os.access(str(CATBOOST_TRAIN_DIR), os.W_OK):
        CATBOOST_CAN_WRITE = True
    else:
        CATBOOST_CAN_WRITE = False
except Exception:
    CATBOOST_CAN_WRITE = False
...
def stage0_preprocessing():
    ...
    if len(all_files) == 0:
        ...
        fraction = 0.1 # Lấy 10% toàn bộ dataset
        ...
        torch.save(torch.tensor(X_train_scaled, dtype=torch.float32), os.path.join(out_dir, "X_train.pt"))
        torch.save(torch.tensor(y_train, dtype=torch.long), os.path.join(out_dir, "y_train.pt"))
        torch.save(torch.tensor(X_test_scaled, dtype=torch.float32), os.path.join(out_dir, "X_test.pt"))
        torch.save(torch.tensor(y_test, dtype=torch.long), os.path.join(out_dir, "y_test.pt"))
...
def stage1_training():
    ...
    models = {
        ...
        "CatBoost_GPU": CatBoostClassifier(
            ...
            allow_writing_files=CATBOOST_CAN_WRITE
        )
    }
    ...
    # Tạo thư mục output theo từng model
    output_folders = {
    ...
    # Xác định số lượng class để xử lý ROC Curve (Nhị phân hay Đa lớp)
    ...
    print("ĐANG TRAIN + ĐÁNH GIÁ CHO TỪNG MODEL...")

    for name, model in models.items():
        print(f"\n{'='*50}")
        print(f"🚀 Đang xử lý model: {name}...")
        ...
        try:
            if model_path.exists() and model_path.stat().st_size > 0:
                print(f"[{name}] Tìm thấy checkpoint model, đang load lại...")
                model = joblib.load(model_path)
            else:
                if model_path.exists() and model_path.stat().st_size == 0:
                    print(f"⚠️ [{name}] File model cũ rỗng, sẽ train lại từ đầu: {model_path}")

                print(f"[{name}] Đang huấn luyện...")
                train_start = time.time()
                model.fit(X_train, y_train)
                print(f"✅ [{name}] Train xong trong {time.time() - train_start:.2f}s")

                joblib.dump(model, model_path)
                print(f"💾 Đã lưu model tại: {model_path}")

        except Exception as e:
            print(f"❌ [{name}] Lỗi khi train/load model: {e}")
        ...
        try:
            ...
            print(f"✅ Đã hoàn thành xử lý cho {name} trong {time.time() - eval_start:.2f}s")

        except Exception as e:
            print(f"❌ [{name}] Lỗi trong quá trình đánh giá: {e}")
            print(traceback.format_exc())
            continue

    print(f"\n{'='*50}")
    print("🎉 HOÀN TẤT! Đã train/đánh giá xong Stage 1")
...
def stage2_feature_selection():
    ...
    if k_neighbors_dynamic < 1:
        ...
        # Giới hạn tỷ lệ sinh dữ liệu tối đa theo 50% class đa số cho bài toán multi-class
        smote_sampling_ratio = 0.5
        class_counts = y_train.value_counts()
        majority_count = int(class_counts.max())
        target_count = max(1, int(np.ceil(majority_count * smote_sampling_ratio)))

        # Với over-sampling, dict phải là số mẫu mục tiêu (>= số mẫu gốc) cho các class cần tăng
        ...
        print(
            f"Đang chạy SMOTE-ENN với k_neighbors = {k_neighbors_dynamic}, "
            f"sampling_strategy = {smote_sampling_ratio} (target_count={target_count}) "
            "(Quá trình này có thể tốn nhiều thời gian)..."
        )

        if not sampling_strategy_dict:
            print("ℹ️ Không có class nào dưới ngưỡng 50% class đa số. Bỏ qua SMOTE-ENN.")
            X_train_resampled, y_train_resampled = X_train_selected, y_train
        else:
            # Khởi tạo SMOTE với thông số tùy chỉnh (sampling_strategy=dict cho multi-class)
            custom_smote = SMOTE(
                random_state=42,
                k_neighbors=k_neighbors_dynamic,
...
def stage4_xai():
    ...
    if importlib.util.find_spec("IPython") is not None:
        try:
            ...
            print(f"⚠️ Không thể khởi tạo SHAP JS trong môi trường hiện tại, tiếp tục chạy bình thường. Chi tiết: {e}")
    else:
        print("ℹ️ Không có IPython, bỏ qua shap.initjs() vì bạn đang chạy script terminal.")
    ...
    try:
        ...
        important_features = torch.load(base_dir_path / "cic_ddos_2018" / "stage2_important_features.pt")
    ...
    def _build_class_name_lookup(csv_files):
        ...
        for csv_file in csv_files:
            try:
                for chunk in pd.read_csv(csv_file, usecols=["Label"], chunksize=250000, low_memory=False):
    ...
    print(f"✅ Đã khôi phục thành công tên thật cho {len(real_feature_names)} features!")
    ...
    for model_file in model_files:
        ...
        if model_name == 'rf':
            ...
            if os.path.exists(shap_cache_path):
                ...
                results = Parallel(n_jobs=n_jobs, prefer=rf_backend, verbose=10)(
                    delayed(_shap_batch_proc)(b.values, tmp_model_path)
                    for b in batches
                )
                ...
                combined_base   = np.concatenate([np.atleast_1d(r.base_values) for r in results], axis=0)
                ...
                print(f"✅ Gộp xong {n_jobs} batch SHAP! Đã lưu cache tại {shap_cache_path}")

        else:
            shap_cache_path = os.path.join(os.getcwd(), f"_shap_{model_name}_cache.joblib")

            # Tạo index trước để dùng cho cả trường hợp có cache lẫn không
            if model_name == 'cat':
                shap_sample_size = min(20000, len(X_test_sample))
                rng = np.random.default_rng(42)
                cat_idx = rng.choice(len(X_test_sample), shap_sample_size, replace=False)
                ...
                if model_name == 'cat':
                    ...
                    try:
                        ...
                        for i in range(0, len(X_shap_input), batch_size):
                            ...
                            gc.collect()
                            print(f"   ✔ Batch {i//batch_size + 1}/{(len(X_shap_input)-1)//batch_size + 1} xong")
                            sys.stdout.flush()

                        if isinstance(shap_batches[0], list):
                            n_classes = len(shap_batches[0])
                            shap_matrix = np.stack(
                                [np.concatenate([b[c] for b in shap_batches], axis=0) for c in range(n_classes)],
                                axis=-1
                            )
                        else:
                        ...
                        if isinstance(base_vals, (list, np.ndarray)):
                            base_values = np.tile(np.array(base_vals), (len(X_shap_input), 1))
                        else:
                        ...
                        traceback.print_exc()
                        sys.stdout.flush()
                        print(">>> Fallback: dùng CatBoost native SHAP để tương thích Linux/Windows...")

                        pool = Pool(X_shap_input, feature_names=list(X_shap_input.columns))
        ...
        plt.savefig(model_out_dir / f"shap_bar_plot_{model_name}.png", bbox_inches='tight', dpi=300)
        ...
        for cls in unique_classes:
            ...
            plt.figure(figsize=(12, 8))
            plt.title(f"SHAP Summary Plot - Tầm quan trọng ({cls_name}) - {model_name.upper()}", fontsize=14)
            if is_multiclass:
            ...
            plt.savefig(model_out_dir / f"shap_summary_class{cls_idx}_{cls_name_slug}_{model_name}.png", bbox_inches='tight', dpi=300)
            plt.close()
            ...
            if len(indices_of_class) > 0:
                ...
                distances = np.linalg.norm(class_samples.values - class_center.values, axis=1)  # Khoảng cách Euclidean
                instance_idx = indices_of_class[np.argmin(distances)]  # Mẫu gần tâm nhất
                predicted_class = int(best_model.predict(X_plot.iloc[[instance_idx]].values)[0])
                predicted_class_name = class_id_to_name.get(predicted_class, f"Class_{predicted_class}")

                plt.figure(figsize=(10, 6))
                if is_multiclass:
                    shap.plots.waterfall(shap_values_obj[instance_idx, :, predicted_class], show=False)
                else:
                ...
                plt.savefig(model_out_dir / f"shap_waterfall_class{cls_idx}_{cls_name_slug}_{model_name}.png", bbox_inches='tight', dpi=300)
                plt.close()
            else:
                print(f"      (Không có gói tin nào thuộc class {cls_name} lọt vào tập sample 2000 dòng để vẽ Waterfall)")
