...
# ==============================================================================
# 0. CÀI ĐẶT THƯ VIỆN & CẤU HÌNH HỆ THỐNG
# ==============================================================================
...
#     # Chỉ cài các gói còn thiếu để tránh cài lại nặng và tốn dung lượng tạm.
...
#     # Torch thường kéo nhiều phụ thuộc rất nặng; ưu tiên CPU wheel cho máy local.
#     install_if_missing("torch", "torch", extra_args=["--index-url", "https://download.pytorch.org/whl/cpu"])
#     # Dành riêng cho GPU LightGBM trên Linux (giữ tinh thần code cũ, nhưng chạy an toàn hơn).
#     if os.name == 'posix' and hasattr(os, "geteuid") and os.geteuid() == 0:
#         # Chỉ chạy khi có quyền root để tránh Permission denied.
...
#     elif os.name == 'posix':
#         print("⚠️ Không có quyền root, bỏ qua bước cấu hình OpenCL hệ thống.")

#     # Pip mới không còn hỗ trợ --install-option, nên dùng cài LightGBM chuẩn để tránh crash.
#     install_if_missing("lightgbm")


def should_install_runtime_requirements() -> bool:
    # Trên Slurm nên dùng venv dựng sẵn; đặt IDPS_SKIP_RUNTIME_PIP=1 để bỏ qua runtime pip.
    return os.environ.get("IDPS_SKIP_RUNTIME_PIP", "0") != "1"
...
def install_requirements():
    ...
    def cleanup_broken_site_packages():
        """Xóa tàn dư package lỗi dạng ~xxx gây warning `Ignoring invalid distribution`."""
        removed = 0
        for site_dir in site.getsitepackages():
            ...
            try:
                ...
                continue
        if removed:
            print(f"  🧹 Đã dọn {removed} mục rác trong site-packages (tiền tố '~').")
    ...
    if not is_installed("torch"):
        ...
        try:
            ...
            if cuda_version and not torch.cuda.is_available():
                ...
                print("  🔄 Đang gỡ sạch và cài lại PyTorch bản GPU ổn định...")
                ...
                print(
                    f"  ✔️  PyTorch đã có + CUDA khả dụng ({torch.cuda.get_device_name(0)})"
                )
            else:
...
# Chạy cài đặt trước khi import thư viện (có thể tắt bằng IDPS_SKIP_RUNTIME_PIP=1)
if should_install_runtime_requirements():
    install_requirements()
else:
    print("ℹ️ Bỏ qua cài package lúc runtime vì IDPS_SKIP_RUNTIME_PIP=1.")
...
def _install_torch_legacy_compat() -> None:
    """Thêm alias cho checkpoint torch cũ tham chiếu `torch.utils.serialization`."""
    if "torch.utils.serialization" in sys.modules:
        return

    compat_module = types.ModuleType("torch.utils.serialization")
    sys.modules["torch.utils.serialization"] = compat_module


_original_torch_load = torch.load


def _torch_load_with_legacy_compat(*args, **kwargs):
    try:
        return _original_torch_load(*args, **kwargs)
    except ModuleNotFoundError as exc:
        if "torch.utils.serialization" not in str(exc):
            raise
        _install_torch_legacy_compat()
        return _original_torch_load(*args, **kwargs)


torch.load = _torch_load_with_legacy_compat
...
os.makedirs(cic_ddos_out_dir, exist_ok=True)
...
def download_dataset():
    ...
    if exit_code == 0:
        ...
        print(
            "❌ Có lỗi xảy ra trong quá trình tải Dataset. Vui lòng kiểm tra lại cấu hình AWS CLI hoặc đường dẫn."
        )
...
def stage0_preprocessing():
    print("\n--- BẮT ĐẦU GIAI ĐOẠN 0: PREPROCESSING (CIC_2019 RECURSIVE) ---")
    import glob

    # 1. Quét tất cả file .csv trong mọi thư mục con (DoS, DDoS, Benign...)
    all_files = glob.glob(os.path.join(csv_dir, "**/*.csv"), recursive=True)

    if len(all_files) == 0:
        print(f"❌ LỖI: Không tìm thấy file CSV nào tại {csv_dir}!")
        return

    print(
        f"✅ Tìm thấy {len(all_files)} file. Bắt đầu đọc dữ liệu bằng chiến thuật Chunking..."
    )
    ...
    rare_classes = [
        ...
        "XSS",  # 3,377 mẫu
        "UPLOADING_ATTACK",  # 1,348 mẫu
        "DNS SPOOFING",  # 74,856 mẫu
        "ARP SPOOFING",  # 82,382 mẫu
    ]
    ...
    for f in all_files:
        ...
        for chunk in chunks:
            ...
            if "Label" in chunk.columns:
                # Tách riêng nhóm phổ biến và nhóm siêu hiếm để bảo toàn dữ liệu hiếm
                rare_data = chunk[chunk["Label"].isin(rare_classes)]
                common_data = chunk[~chunk["Label"].isin(rare_classes)]

                # Lấy 10% cho các class phổ biến (Giữ đúng tỉ lệ phân bố nhãn)
    ...
    # Bí kíp 1: Loại bỏ cột thừa thiếu trên 50% dữ liệu
    df_full.dropna(thresh=len(df_full) * 0.5, axis=1, inplace=True)

    # Bí kíp 2: Xử lý Infinity về 0 để không mất dòng tấn công
    df_full.replace([np.inf, -np.inf], 0, inplace=True)

    # Xóa dòng trống và dòng header lặp lại
    ...
    # Bí kíp 3: Ép kiểu số và chặn vô cực sau khi ép kiểu
    ...
    # Chia tập dữ liệu đảm bảo tỉ lệ phân bố (Stratify)
    X_train, X_test, y_train, y_test = train_test_split(
        X, y_encoded, test_size=0.2, random_state=42, stratify=y_encoded
    )

    print("⚖️ Đang chuẩn hóa MinMaxScaler...")
    scaler = MinMaxScaler()
    ...
    print("💾 Đang lưu file .pt vào thư mục đầu ra...")
    torch.save(
        torch.tensor(X_train_scaled, dtype=torch.float32),
        os.path.join(cic_ddos_out_dir, "X_train.pt"),
    )
    torch.save(
        torch.tensor(y_train, dtype=torch.long),
        os.path.join(cic_ddos_out_dir, "y_train.pt"),
    )
    torch.save(
        torch.tensor(X_test_scaled, dtype=torch.float32),
        os.path.join(cic_ddos_out_dir, "X_test.pt"),
    )
    torch.save(
        torch.tensor(y_test, dtype=torch.long),
        os.path.join(cic_ddos_out_dir, "y_test.pt"),
    )

    print("✅ HOÀN TẤT GIAI ĐOẠN 0!")
...
def stage1_training():
    ...
    path_rf = os.path.join(
        ...
        "stage1_output",
        "RandomForest_GPU_model.joblib",
    )
    path_xgb = os.path.join(
        cic_ddos_out_dir,
        "output_cic_ddos_2018",
        "stage1_output",
        "XGBoost_GPU_model.joblib",
    )
    path_lgb = os.path.join(
        cic_ddos_out_dir,
        "output_cic_ddos_2018",
        "stage1_output",
        "LightGBM_CPU_model.joblib",
    )
    path_cat = os.path.join(
        cic_ddos_out_dir,
        "output_cic_ddos_2018",
        "stage1_output",
        "CatBoost_GPU_model.joblib",
    )
    ...
    for name, path in model_paths.items():
        ...
        try:
            ...
            print(
                f"❌ Không tìm thấy file model tại {path}. Vui lòng kiểm tra lại đường dẫn! Bỏ qua model này."
            )
            continue
        ...
        # Lưu .txt
        with open(
            os.path.join(out_dir, f"metrics_{name}.txt"), "w", encoding="utf-8"
        ) as f:
        ...
        with open(
            os.path.join(out_dir, f"metrics_{name}.json"), "w", encoding="utf-8"
        ) as f:
...
def stage2_feature_selection():
    ...
    rf_finder = RandomForestRegressor(
        n_estimators=15, max_depth=10, max_samples=0.05, random_state=42, n_jobs=-1
    )
    ...
    print("\n" + "=" * 60)
    print(
        f"🎯 CON SỐ THRESHOLD ĐỂ LẤY ĐÚNG {target_features} FEATURES LÀ: {magic_threshold}"
    )
    print("=" * 60 + "\n")

    print("--- BẮT ĐẦU GIAI ĐOẠN 2 (PYTORCH TENSORS) ---")

    def load_stage1_tensors() -> (
        tuple[pd.DataFrame, pd.DataFrame, pd.Series, pd.Series]
    ):
        """Load dữ liệu từ các file .pt của Stage 1 và chuyển sang Pandas để xử lý."""
        try:
            X_train_tensor = torch.load(os.path.join(cic_ddos_out_dir, "X_train.pt"))
            X_test_tensor = torch.load(os.path.join(cic_ddos_out_dir, "X_test.pt"))
            y_train_tensor = torch.load(os.path.join(cic_ddos_out_dir, "y_train.pt"))
            y_test_tensor = torch.load(os.path.join(cic_ddos_out_dir, "y_test.pt"))
            ...
            print(
                f"Lỗi: Không tìm thấy file .pt. Vui lòng kiểm tra lại đường dẫn! Chi tiết: {e}"
            )
            raise
    ...
    if os.path.exists(checkpoint_path):
        ...
        rf_regressor = RandomForestRegressor(
            n_estimators=30, max_depth=10, max_samples=0.1, random_state=42, n_jobs=-1
        )
        ...
        important_features = X_train.columns[
            rf_regressor.feature_importances_ >= threshold
        ]
        print(
            f"Số lượng features giữ lại: {len(important_features)} / {len(X_train.columns)}"
        )
    ...
    # print("\nĐang chạy SMOTE-ENN (Quá trình này có thể tốn nhiều thời gian)...")
    ...
    # 3. Xử lý mất cân bằng dữ liệu với SMOTE (Thay thế SMOTE-ENN)
    print("\n--- CHUẨN BỊ SMOTE ---")
    ...
    if k_neighbors_dynamic < 1:
        ...
        print(
            "Bạn cần quay lại Stage 0 để điều chỉnh tỷ lệ lấy mẫu (fraction) cao hơn cho các class này."
        )
    else:
        print(f"Đang chạy SMOTE với k_neighbors = {k_neighbors_dynamic}...")

        # THAY ĐỔI CHÍNH: Khởi tạo SMOTE trực tiếp
        # Chúng ta bỏ qua bước khởi tạo SMOTEENN
        smote = SMOTE(random_state=42, k_neighbors=k_neighbors_dynamic)

        # Thực hiện fit_resample trực tiếp bằng đối tượng smote thay vì smote_enn
        X_train_resampled, y_train_resampled = smote.fit_resample(
            X_train_selected, y_train
        )

        print(f"Kích thước tập train ban đầu: {X_train_selected.shape}")
        print(f"Kích thước tập train sau cân bằng (SMOTE): {X_train_resampled.shape}")
    ...
    torch.save(
        list(important_features), os.path.join(save_dir, "stage2_important_features.pt")
    )

    # ==============================================================================
    # BỔ SUNG: FULL EVALUATION CHO STAGE II (Y HỆT STAGE I)
    # ==============================================================================
    # --- KHAI BÁO VÀ TẢI DỮ LIỆU CHO STAGE II EVALUATION ---
    print("📥 Đang tải dữ liệu Stage II đã xử lý để chạy Evaluation...")

    try:
        # 1. Tải các tensors đã lưu từ Stage 2
        X_train_stage2_tensor = torch.load(
            os.path.join(cic_ddos_out_dir, "stage2_X_train.pt"), weights_only=False
        )
        y_train_stage2_tensor = torch.load(
            os.path.join(cic_ddos_out_dir, "stage2_y_train.pt"), weights_only=False
        )
        X_test_stage2_tensor = torch.load(
            os.path.join(cic_ddos_out_dir, "stage2_X_test.pt"), weights_only=False
        )
        y_test_stage2_tensor = torch.load(
            os.path.join(cic_ddos_out_dir, "stage2_y_test.pt"), weights_only=False
        )

        # 2. Chuyển đổi sang định dạng Numpy/Pandas để các Model Scikit-learn/XGBoost xử lý
        X_train_resampled = X_train_stage2_tensor.numpy()
        y_train_resampled = y_train_stage2_tensor.numpy()
        X_test_selected = X_test_stage2_tensor.numpy()
        y_test = y_test_stage2_tensor.numpy()

        max_samples = (
            20000000  # Giới hạn 2 triệu dòng là cực kỳ an toàn và đủ chính xác
        )
        if len(X_train_resampled) > max_samples:
            print(f"⚠️ Dữ liệu quá lớn ({len(X_train_resampled)} dòng).")
            print(
                f"🚀 Đang lấy mẫu ngẫu nhiên {max_samples} dòng để tránh lỗi 'bad allocation'..."
            )
            np.random.seed(42)
            indices = np.random.choice(
                len(X_train_resampled), max_samples, replace=False
            )
        ...
        print(
            f"❌ Lỗi: Không tìm thấy file dữ liệu Stage 2 (.pt). Vui lòng kiểm tra thư mục {cic_ddos_out_dir}"
        )
        return
    except Exception as e:
        print(f"❌ Đã xảy ra lỗi khi nạp dữ liệu: {e}")
        return
    # ------------------------------------------------------
    print("\n" + "=" * 50)
    print("🚀 ĐANG CHẠY FULL EVALUATION CHO STAGE II...")
    print("=" * 50)

    # 1. Tạo thư mục output riêng cho Stage 2
    stage2_out_dir_full = os.path.join(base_dir, "output_stage2_full_reports")
    os.makedirs(stage2_out_dir_full, exist_ok=True)

    # 2. Cấu hình các Model (Đã tối ưu để tránh MemoryError)
    # Lưu ý: n_jobs=2 và max_samples là 'chìa khóa' để chạy được trên 12 triệu dòng
    # 2. Cấu hình đầy đủ 4 Model (Y hệt Stage I nhưng tối ưu cho dữ liệu lớn)
    num_classes = len(np.unique(y_train_resampled))
    objective = "multi:softmax" if num_classes > 2 else "binary:logistic"

    models_s2 = {
        # 1. Random Forest (Dùng XGBoost Random Forest để tận dụng GPU thay vì sklearn CPU)
        "RandomForest_GPU_2": XGBRFClassifier(
            n_estimators=100,
            tree_method="hist",  # Thuật toán tối ưu cho GPU
            device="cuda",  # Bật GPU
            objective=objective,
        ...
        # 4. CatBoost (Chạy CPU)
        "CatBoost_GPU_2": CatBoostClassifier(
            iterations=100,
            task_type="GPU",
            # task_type="CPU",
            devices="0",  # Chạy trên GPU đầu tiên
    ...
    # Lấy tên các class để vẽ hình
    classes_s2 = np.unique(y_test)
    n_classes_s2 = len(classes_s2)
    y_test_bin_s2 = (
        label_binarize(y_test, classes=classes_s2) if n_classes_s2 > 2 else None
    )

    for name, model in models_s2.items():
        print(f"\n[Stage II] 🟢 Đang xử lý: {name}...")
        model_folder = os.path.join(stage2_out_dir_full, f"results_{name}")
        ...
        with open(os.path.join(model_folder, f"metrics_{name}.txt"), "w") as f:
            f.write(report_text)
        ...
        )  # Màu xanh lá để phân biệt với Stage I
        plt.title(f"Confusion Matrix Stage II - {name}")
        plt.savefig(os.path.join(model_folder, "confusion_matrix.png"), dpi=300)
        plt.close()

        # --- VẼ ROC CURVE ---
        if y_pred_proba is not None and n_classes_s2 > 2:
            plt.figure(figsize=(10, 8))
            for i in range(n_classes_s2):
                fpr, tpr, _ = roc_curve(y_test_bin_s2[:, i], y_pred_proba[:, i])
                plt.plot(fpr, tpr, label=f"Class {i}")
            plt.plot([0, 1], [0, 1], "k--")
            plt.title(f"ROC Curve Stage II - {name}")
            plt.legend()
            plt.savefig(os.path.join(model_folder, "roc_curve.png"), dpi=300)
            plt.close()

        # Lưu Model
        joblib.dump(model, os.path.join(model_folder, f"{name}.joblib"))
        print(f"✅ Hoàn thành và lưu kết quả cho {name}")
        del model
...
def stage3_hyperopt():
    ...
    hyperopt_checkpoint_path = (
        Path(stage3_checkpoint_dir) / "stage3_hyperopt_checkpoint.joblib"
    )
    hyperopt_best_params_path = (
        Path(stage3_checkpoint_dir) / "stage3_hyperopt_best_params.joblib"
    )
    ...
    try:
        ...
        X_train_resampled = pd.DataFrame(
            X_train_tensor.numpy(), columns=important_features
        )
    ...
    HYPEROPT_CHECKPOINT_PATH = (
        base_dir_path / "checkpoint_rf_stage3" / "stage3_hyperopt_checkpoint.joblib"
    )
    HYPEROPT_BEST_PARAMS_PATH = (
        base_dir_path / "checkpoint_rf_stage3" / "stage3_hyperopt_best_params.joblib"
    )
    ...
    def _make_objective(X_chunk, y_chunk):
        def objective(params):
            ...
            score = cross_val_score(
                model, X_chunk, y_chunk, scoring="f1_macro", cv=3
            ).mean()
    ...
    hyperopt_data_ranges = _build_chunk_ranges(
        hyperopt_total_samples, HYPEROPT_DATA_CHUNK_SIZE
    )
    ...
    if os.path.exists(HYPEROPT_CHECKPOINT_PATH):
        ...
        completed_evals = int(
            hyperopt_checkpoint.get("completed_evals", len(trials.trials))
        )
        next_data_chunk_idx = int(hyperopt_checkpoint.get("next_data_chunk_idx", 0))
        print(
            f"[HYPEROPT] Resume từ {completed_evals}/{HYPEROPT_MAX_EVALS} evaluations."
        )
    ...
    while completed_evals < HYPEROPT_MAX_EVALS:
        ...
        target_evals = min(
            completed_evals + HYPEROPT_EVAL_CHUNK_SIZE, HYPEROPT_MAX_EVALS
        )
        best = fmin(
    ...
    final_rf = RandomForestClassifier(
        ...
        max_depth=30,  # Giảm xuống từ 44
        max_features=int(best_rf["max_features"]),
        min_samples_leaf=int(best_rf["min_samples_leaf"]),
        min_samples_split=int(best_rf["min_samples_split"]),
        n_estimators=int(best_rf["n_estimators"]),
        max_samples=0.1,  # CHỐNG LỖI RAM: Chỉ lấy 10% dữ liệu cho mỗi cây
        random_state=106,
        n_jobs=2,  # Giảm số luồng để tiết kiệm RAM
    )
    ...
    if xgb_params_file.exists():
        ...
        def objective_xgb(params):
            ...
            score = cross_val_score(
                model, X_train_tuning, y_train_tuning, scoring="f1_macro", cv=3
            ).mean()
            return {"loss": -score, "status": STATUS_OK}
